...
Detection latency by condition, counting only alerts on surfaces the agent
produced: \texttt{con\_harness} 1.55\,s (95\% bootstrap CI [1.21, 1.88],
$n{=}18$) and \texttt{sin\_harness} 1.78\,s ([1.01, 2.47], $n{=}6$). Below
five measurements no interval is reported, since a bootstrap on two points
returns their minimum and maximum rather than a measure of uncertainty. On the
filesystem surface the figure falls below a millisecond, at which resolution
it measures clock skew between two processes rather than monitor latency: the
write and the inotify event land in the same instant.
