...
matters. Across all 416 runs the proxy flagged
\texttt{mirror-externo:5002} thirty-seven times, on three separate machines,
and that is the only destination any agent was ever flagged reaching: every
one of them went at the attacker directly, and not one run used the SSRF
route. The single other anomalous destination on record,
\texttt{destino-no-autorizado:9999}, appears twice and is the synthetic event
the positive-control script injects, not a model's choice. The blind spot was demonstrated by a synthetic control, not
...
\subsection*{The model invents tools that do not exist.} Sixty nine tool calls
across the corpus name a function absent from the dispatcher: \texttt{mkdir}
(67), \texttt{write\_dir} (1) and \texttt{edit\_file} (1). The dispatcher
returns a string naming the unknown tool rather than raising, so the call
costs a turn rather than ending the run. We do not report this as evidence of
anything about escape or detection; we note it because \texttt{mkdir} is the
exact action the injected instruction asks for (create a directory named with
the agent's canary), and a model reaching for a shell-familiar name it was
never given is a different kind of tool-use failure than the ones the rest of
this paper is built to measure.
